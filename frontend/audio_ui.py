...
def seed_update(seed, seed_fixed, output_state):
    if seed_fixed == "🎲" and output_state is not None:
...
def edit_workflow(model, prompt_input, neg_prompt_input, scale, steps, cfg, length, batch_size, seed, input_imgs=None, sampler_name=None, quality=None, sampling_shift=None, reinhard=None):
    ...
    # 暂时没有
        ...
        workflow["3"]["inputs"]["cfg"] = cfg
        workflow["3"]["inputs"]["sampler_name"] = sampler_name
        workflow["17"]["inputs"]["batch_size"] = batch_size

        workflow["17"]["inputs"]["seconds"] = length
        workflow["63"]["inputs"]["quality"] = quality

        workflow["45"]["inputs"]["shift"] = sampling_shift
        
        workflow["61"]["inputs"]["multiplier"] = reinhard

    return workflow



def video_generate(launch_state, img_display_state, model, prompt_input, neg_prompt_input, scale, steps, cfg, length, batch_size, seed, queue_size, input_imgs=None, sampler_name=None, quality=None, sampling_shift=None, reinhard=None):
    ...
    for i in range(queue_size):
        ...
        workflow = edit_workflow(
            workflow_model_name, prompt_input, neg_prompt_input, scale, steps, cfg, length, batch_size, seed, input_imgs,
            sampler_name, quality, sampling_shift, reinhard
        )
    ...
    return display_outputs, gr.update(visible=False)
    ...
    with gr.Blocks(theme="soft", js=js_func, css=generate_btn_css) as demo:
        ...
        with gr.Row():
            with gr.Column(scale=3):
                ...
                with gr.Group():
                    scale = gr.Dropdown(choices=resolution_list, label="分辨率(宽×高)", visible=False)
                    length = gr.Slider(value=150.0, label="生成时长(秒)", minimum=1, maximum=320.0, step=0.1)
                    reinhard = gr.Slider(value=1.00, label="莱因哈德色调映射(乘数)", minimum=0, maximum=100.0, step=0.01)
                    sampling_shift = gr.Slider(value=5.0, label='模型采样移位', minimum=0, maximum=64, step=0.1, visible=True)
                    steps = gr.Slider(value=50, label='迭代步数 Steps', minimum=1, maximum=128, step=1)
                    ...
                    quality = gr.Dropdown(choices=["128k", "320k"], value="128k", label="输出质量",)
            ...
            with gr.Column(scale=10, min_width=320): # , scale=10
                with gr.Row():
                    ...
                    neg_prompt_input = gr.Textbox(
                        placeholder="歌词", show_label=False, container=False, lines=8, max_lines=8, scale=4, visible=neg_prompt_display
                    )
                ...
                def show_audio(paths):
                    if paths:
                        with gr.Group():
                            for p in paths:
                                name = os.path.basename(p)
                                gr.Audio(
                                    value=p, label=name, interactive=False, show_download_button=True,
        ...
        clear_outputs_kwargs = {
            "fn": clear_outputs,
            "inputs": [],
            "outputs": [output_state, all_output]
        }
        
        generate_kwargs = {
            "fn": video_generate,
            "inputs": [launch_state, img_display_state, model, prompt_input, neg_prompt_input, scale, steps, cfg, length, batch_size, seed, queue_size, input_img] + [sampler_name, quality, sampling_shift, reinhard],
            "outputs": [output_state, all_output],
            "show_progress": "full",
            "show_progress_on": all_output
        }
        
        seed_update_kwargs = {
            "fn": seed_update,
            "inputs": [seed, seed_fixed, output_state],
            "outputs": [seed]
        }
